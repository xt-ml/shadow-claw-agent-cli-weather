...
$SC agent run \
  ...
  -y \
  -o "$OUT" \
  "Use fetch_url to call https://api.open-meteo.com/v1/forecast?latitude=${LOC_LAT}&longitude=${LOC_LON}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,snowfall,weather_code,cloud_cover,wind_speed_10m,wind_direction_10m,wind_gusts_10m,dew_point_2m&temperature_unit=celsius&wind_speed_unit=kmh&precipitation_unit=mm&timezone=${LOC_TZ_ENCODED} and then write a clear, friendly current weather report for ${LOC_NAME}. Include: conditions summary, temperature and feels-like, humidity, dew point, wind speed and direction, any precipitation, and a short 1-2 sentence outlook note. Format as clean markdown with the current date/time as the heading."
